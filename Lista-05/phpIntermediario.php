...
<body>
    ...
    <form method="post">
        ...
        <input type="text" id="nota4" name="nota4" required><br>
    ...
    <form method="post">
        ...
        <?php
            ...
            if ($_SERVER["REQUEST_METHOD"] == "POST") {
                for ($i = 1; $i <= 5; $i++) {
                    $itemCompra = "item$i";
                    if (isset($_POST[$itemCompra]) && !empty($_POST[$itemCompra])) {
                        $item = $_POST[$itemCompra];
                        $listaDeCompras[] = $item;
                    }
                }
                sort($listaDeCompras);
            ...
            for ($i = 1; $i <= 5; $i++) {
                $itemCompra = "item$i";
                echo "<label for='$itemCompra'>Item $i:</label> ";
                echo "<input type='text' name='$itemCompra' value='" . (isset($_POST[$itemCompra]) ? $_POST[$itemCompra] : "") . "'><br>";
            }
        ?><br>
        <input type="submit" value="Enviar">
    </form>
